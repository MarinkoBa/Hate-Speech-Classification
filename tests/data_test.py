...
class TestDataCollection(unittest.TestCase):
    ...
    def test_load_data(self):
        ...
        self.assertIsInstance(load_data(os.path.join(os.path.pardir, 'src', 'data', 'tweets.csv')),
                              pd.core.frame.DataFrame)

        self.assertIsInstance(get_datasets(os.path.join(os.path.pardir, 'src', 'data', 'labeled_data.csv'),
                                           os.path.join(os.path.pardir, 'src', 'data',
        ...
        self.assertIsInstance(concatenate_datasets(os.path.join(os.path.pardir, 'src', 'data', 'tweets.csv'),
                                                   self.df2, self.df3),
                              pd.core.frame.DataFrame)
