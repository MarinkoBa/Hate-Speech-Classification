...
def cross_validate(x, y, n_splits=10):
    ...
    for i, (train, test) in enumerate(kf.split(x)):
        ...
        acc_dec_tree = metrics.accuracy_score(y_test, y_pred_dec_tree)
        prec_dec_tree = metrics.precision_score(y_test, y_pred_dec_tree)
        rec_dec_tree = metrics.recall_score(y_test, y_pred_dec_tree)
        ...
        acc_ran_for = metrics.accuracy_score(y_test, y_pred_ran_for)
        prec_ran_for = metrics.precision_score(y_test, y_pred_ran_for)
        rec_ran_for = metrics.recall_score(y_test, y_pred_ran_for)
        ...
        acc_log_reg = metrics.accuracy_score(y_test, y_pred_log_reg)
        prec_log_reg = metrics.precision_score(y_test, y_pred_log_reg)
        rec_log_reg = metrics.recall_score(y_test, y_pred_log_reg)
